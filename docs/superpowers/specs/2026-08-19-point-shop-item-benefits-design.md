...
| 쿠폰 코드 이행 | **구매 트랜잭션에서 미배정 코드 1개 자동 배정 → 주문 즉시 지급 완료** (수동 처리 없음) |
| 취소·환불 | 혜택형(쿠폰·끌올·연장)은 **취소·환불 불가**(코드 공개·즉시 효력) — 구매 확인 다이얼로그에 사전 고지. 수동 지급형만 현행처럼 취소·자동 환불 |
...
| 수동 지급형 재고 복원 | 운영자 취소·환불 시 **재고 +1 복원**(같은 트랜잭션·조건부 원자 UPDATE) |
| benefit_type 변경 잠금 | 판매 이력(주문 존재) 또는 등록 코드가 있는 아이템은 **benefit_type 변경 금지**(폼 비활성+서버 거부). 가격·이름·이미지 등은 자유 |
| 만료 포인트 소멸 | 만료된 보유 아이템의 포인트 소멸은 **의도된 정책**(취소 불가·기한 명시 고지로 갈음). 만료 임박 알림은 스코프 밖 |
| 역할 전환·조직 이탈 | 사용 시점 **자격 재검사로 거부** + 안내(회수·환불 없음) |
...
| `coupon_code` | 쿠폰 코드 풀 자동 배정 | — | 구매 즉시 코드 1개 배정·지급 완료 | **비활성(코드 수 종속)** | 미적용(외부 유효기간) |
...
- 상태값(모두 `text`): `pending`·`completed`·`canceled`(현행) + `owned`·`used`(신규).
  ...
  - 쿠폰형: 구매 즉시 `completed`(코드 배정 = 지급 완료).
  - 끌올·연장형: 구매 시 `owned` → 사용 시 `used`.
...
1. 구매 트랜잭션에서 계정 락 획득 후 **미배정 코드 1행 선점**(`FOR UPDATE SKIP
   LOCKED`, §5) → 없으면 품절 에러.
2. 주문 insert(`status = completed`, benefit 스냅샷) → 코드 행에 `order_id`·
   `assigned_at` 세팅 → 원장 `point_shop_purchase` −가격.
3. 취소·환불 불가(코드가 구매 즉시 공개됨) — 구매 확인 다이얼로그에 명시.
4. 구매자 화면(마이페이지): 코드 표시 + 복사 버튼 + "사용 완료" 토글(양방향,
   `markCouponUsed`).
5. 재고·품절: 미배정 코드 수로 노출/품절 표시. 운영자가 언제든 추가 충전.
...
- 구매 확인 다이얼로그: "취소·환불 불가, 구매 후 보유함에서 직접 사용" 명시.
...
3. **사용 시점 자격 재검사**: 구매 후 역할 전환(구인→구직)·조직 이탈로 대상 공고를
   가질 수 없게 된 경우 서버가 거부하고 안내 문구를 반환한다(회수·환불 없음 —
   구매·보유 자체는 유효했음). 대상 공고 소유(자기 조직)·게시 중·배너형 제외·연장
   종료일 조건도 이 시점에 다시 확인한다.
...
별도 cron 없음 — 읽기 시 파생 판정. 만료돼도 환불 없음(사전 고지).

**혜택형 공통:** 운영자 주문 탭의 취소 대상 아님(버튼 미노출·서버 거부).
...
| `purchase({ itemId })` | benefit_type 분기: 수동=`pending`, 쿠폰=코드 배정·`completed`, 끌올·연장=`owned`. 자격·재고·품절·잔액 검증 |
| `myOrders` | 구매 내역 + **보유함(owned)** 포함으로 확장. 쿠폰 코드·남은 기한·사용 대상 표시 |
| `useBenefit({ orderId, jobPostId })` | 보유 건 사용(끌올 구매 행 생성 또는 노출 연장, `owned→used`) |
| `markCouponUsed({ codeId, used })` | 쿠폰 "사용 완료" 양방향 토글 |
...
| `listOrders` 확장 | 혜택형 주문 표시(취소 버튼은 수동 지급형에만) |

### 서비스 순수 함수(`test/services` 단위 테스트 대상)

- `resolvePurchase` 확장: audience 자격·재고·품절 판정 추가.
- `resolveOrderTransition` 확장: `owned → used` 전이 + 만료(`usable_until`) 가드.
...
- 주문 행 `FOR UPDATE` + `owned` 상태 전용 전이 가드(멱등) — `used`·`canceled`·
...
- (권고) 운영자 `adjustJobPostExposure`도 대상 행 `FOR UPDATE` 또는 동일 원자 갱신
  형태로 맞추면 두 경로 간 유실이 원천 차단된다 — 이번 스코프에서 함께 검토.
...
- 유형별 안내: 혜택형은 "취소·환불 불가", 쿠폰은 "구매 즉시 코드 지급(취소 불가)",
  끌올·연장은 "보유함에서 직접 사용" 고지.
...
- 보유 중(`owned`): "사용하기" 버튼 + 남은 기한(`usable_until`) 또는 무기한 표시,
  만료 건은 만료 뱃지.
- 쿠폰: 코드 + 복사 버튼 + "사용 완료" 토글.
- 이력: 완료·취소·사용 완료 등 상태 라벨 맵으로 렌더.
...
- **마이그레이션 1건**(끌올 옵션 0080·프리미엄 정원 등과 같은 관례):
  ...
  - `bambi_point_shop_order` 컬럼 8개 추가.
...
- 매뉴얼 동기화(구직자·구인자·운영자): 혜택 아이템 구매·보유함·사용하기, 쿠폰 코드
  수령·사용 완료, 운영자 혜택 아이템·코드 관리.
...
- 코드 CSV 내보내기.
- 보유 아이템 **만료 임박 알림**(만료 시 포인트 소멸은 의도된 정책이라 알림만 이연).
- (권고 검토) 운영자 `adjustJobPostExposure`의 원자 갱신 전환은 이번에 함께 볼 수
  있으나, 미이관 시에도 포인트몰 경로 자체는 원자 갱신이라 안전.
...
노출 조정의 갱신 유실. 각각 계정 advisory lock, `FOR UPDATE SKIP LOCKED` 코드 선점,
주문 행 `FOR UPDATE` + `owned` 멱등 전이, `exposure_ends_at` 원자 갱신으로 봉쇄하며,
단위 테스트와 코드 리뷰 체크리스트의 최상단에 둔다.
