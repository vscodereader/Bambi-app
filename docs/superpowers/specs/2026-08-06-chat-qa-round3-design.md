...
- §G 보정: 면접 일정은 방 삭제와 무관하게 유지한다(listMyUpcomingInterviews의 삭제 방
  제외를 철회 — 면접은 현실의 약속). 삭제된 방의 카드 클릭은 방 오류 카드로 떨어진다.
- 방 오류 카드: "로그인 상태를 확인해 주세요." 문구 제거, "채팅 목록으로" 버튼 그림자 제거.
- 내 정보 메뉴 역할 숨김: 구인자·운영자(admin)는 "예정된 면접" 숨김, 운영자는 추가로
  "차단한 상대"·"내 신고 내역"·"고객센터" 숨김(메뉴 노출만, 직접 URL은 범위 밖).
- §H 재개정: 가입 폼을 혼합 배치로 전환 — 닉네임|아이디 / 이메일 / 비밀번호|비밀번호 확인 /
  가입 유형(컨테이너 쿼리 @md 2열, 좁으면 1열 세로 동일 순서, items-start 정렬).
- TC-3 핀 잔존은 사용자 지시로 보류(4차에서 미수정).
