...
> **For agentic workers:** 이 문서는 아래 네 가지 비색상 UI 배치 변경만 구현한다. 다크모드·색상·테마 토큰과 그 관련 파일은 수정하지 않는다.

**Goal:** 운영자 사용자 관리 선택 열을 출석 관리와 같은 여백·정렬로 맞추고, 공용 헤더 검색 버튼 크기를 다른 아이콘 버튼과 통일하며, 모바일 신고창의 과도한 폭·세로 길이를 기존 디자인 토큰으로 줄인다. 운영자 공고 검수 상세 카드는 헤더 콘텐츠와 같은 좌우 기준선 안에 배치한다.

**Architecture:** 최신 `develop`에서 분리된 `fix/table-selection-search-control-layout` 브랜치 하나를 사용한다. 사용자 선택 열은 출석 관리 표의 기존 `w-10`, TableCell `p-2`, `flex justify-center` 패턴을 그대로 재사용한다. 검색 버튼은 별도 크기 override를 제거하고 공통 Button `icon-lg`만 사용한다. 신고창은 공통 Dialog의 데스크톱 기본값을 유지하고 `max-md:` 기존 Tailwind 토큰만 추가한다. 공고 검수 상세 카드는 운영자 셸의 폭 상수를 바꾸지 않고, 헤더가 같은 셸 안에서 사용하는 기존 데스크톱 좌우 여백 `px-6`과 대응하는 `lg:mx-6`만 적용한다.
...
- 공고 검수 상세 카드 외곽선은 데스크톱에서 헤더 콘텐츠의 좌우 기준선 안에 맞춘다.
- 운영자 공통 셸 폭, 다른 운영자 페이지, 모바일 검수 상세의 전체 폭은 변경하지 않는다.
...
## Task 4: 공고 검수 상세 카드 폭을 헤더 기준선에 정렬

**Files:**

- Modify: `apps/web/src/components/bambi/screens/moderator.tsx`
- Create: `apps/web/test/components/bambi/moderator-queue-detail-width.test.ts`

**Implementation:**

- 스크린샷에서 공고 검수 상세 카드가 운영자 헤더 콘텐츠 기준선을 양쪽으로 넘어가는 현상을 수정한다.
- 카드의 데스크톱 스타일에 기존 6단계 좌우 여백 토큰 `lg:mx-6`을 적용한다.
- `APP_CONTENT_WIDTH`와 `APP_CONTENT_MAX_W`는 이미 헤더·본문 공통 폭을 담당하므로 변경하지 않는다.
- 카드 내부 padding, 모바일 전체 폭, 색상, 다크모드 토큰, 검수 동작은 변경하지 않는다.

- [x] 공고 검수 상세 카드 데스크톱 좌우 여백 구현
- [x] 공고 검수 상세 카드 폭 회귀 테스트
...
3. `fix: 공고 검수 상세 카드 폭을 헤더 기준선에 정렬`

사용자가 요청한 네 비색상 배치 변경은 하나의 브랜치로 묶되, 설계와 구현 커밋을 분리한다. `--force`를 사용하지 않고 최종 머지는 직접 수행하지 않는다.
