...
**Goal:** 웹의 공개·구직자·업주·운영자·로그인·고객센터·편집기 화면에 사용자가 직접 전환하고 재방문에도 유지되는 다크모드를 제공한다. 다크모드는 흰 계열 페이지·카드 표면과 일반 검은 계열 문자를 어두운 표면·밝은 문자로 전환하되, 브랜드/상태 버튼과 테두리의 기존 색상은 유지한다.
...
- 브랜드·상태·기능 버튼의 배경색과 그 배경에 대응하는 기존 글자색은 다크모드에서도 유지한다. 특히 흰 버튼은 흰 배경과 어두운 글자를 함께 유지해 대비를 깨지 않는다.
...
- `packages/ui` 프리미티브의 기존 `dark:` 분기는 outline 버튼 배경·destructive 배지·입력·탭 등도 바꾼다. 버튼/상태 색 유지 정책과 충돌하는 분기는 공통 프리미티브에서 제거하거나 같은 시각값으로 고정하고, 입력·탭처럼 표면 전환 대상인 요소만 새 시맨틱 토큰을 따르게 한다.
...
- `git diff --check`: 통과.
- 실제 브라우저 데스크톱 `/jobs`: 라이트에서 `body #fff / text #111827`, 다크에서 페이지 `#0b1018`, 카드 `#0f1620`, 본문 흰색으로 전환됨을 확인했다. 테두리는 두 모드 모두 `#eef0f3`, 흰 outline 버튼은 `#fff / #111827`로 유지됐다.
