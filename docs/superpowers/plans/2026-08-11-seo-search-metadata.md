...
- 공용 푸터의 `지역별 채용 정보`, `커뮤니티 게시판`, `공지사항`, 고용노동부 `체불사업주 명단` 링크.
...
- `/jobs`: 전국 지역·업종별 밤알바·유흥알바·룸알바 채용 정보.
- `/jobs/[region]`: 지역명 + 밤알바/유흥알바 채용 의도.
...
- `/board`: `밤알바·여성알바 정보 커뮤니티 - 밤비알바` 성격을 실제 공개 게시판 범위와 함께 설명한다.
...
- [ ] 체불사업주 명단 기존 링크의 최종 응답과 라벨을 확인한다.
