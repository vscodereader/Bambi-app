...
- Expo SDK 56 / Expo Go, Android Studio AVD `emulator-5554`에서 서버(23000), web(23001), native를 함께 실행했다.
- 홈 미리보기, 게시판 허브, 공지·베스트·자유수다 목록, 검색, 게시글 생성·상세·추천·댓글·답글·수정·삭제와 신고 다이얼로그를 실제 API 왕복으로 확인했다.
- 남성 인증 구직자는 공지·비밀글 범위만 노출되고, 미인증 구직자는 일반 게시판 진입 시 안내 후 홈으로 복귀하는 것을 확인했다.
- 실제 QA에서 일반 구직자에게 공지 글쓰기 버튼이 노출되던 문제와 미인증 계정이 범용 조회 오류를 보이던 문제를 수정했다.
- `pnpm --filter native test`: 39 files / 395 tests 통과. native 타입 검사, Ultracite, `git diff --check` 통과.

## 2026-09-11 수다방 탐색 UI 변경

- 수다방 탭에서는 공용 구직자 헤더의 로고·워드마크를 `수다방` 제목으로 교체하고 검색 액션만 숨긴다. 포인트몰과 알림 액션은 기존 역할 정책과 경로를 유지한다.
- 탭 본문의 중복 `수다방` 제목과 설명은 제거하고 게시판 목록을 바로 시작한다.
- 수다방 게시판 카드의 글 개수 표시는 제거한다. 같은 자리에 접근성 라벨이 있는 오른쪽 화살표를 표시하고, 카드 헤더를 누르면 해당 게시판 전체 목록으로 이동한다.
- 구직자 메인의 수다방 최상위 `전체 보기` 문구는 오른쪽 화살표로 교체하며 기존 수다방 탭 경로를 유지한다.
- 메인 미리보기는 운영자가 `main` surface에 지정한 순서를 변경하지 않고 게시판 두 개를 세로로 묶는다. 각 게시판은 최신 글을 최대 두 개만 표시한다.
- 게시판 두 개 묶음은 가로 스냅 스크롤로 이동한다. 첫 묶음에서는 다음 묶음, 중간 묶음에서는 이전·다음 묶음, 마지막 묶음에서는 이전 묶음 일부가 가장자리에 보이도록 화면 너비에서 공통 여백을 계산한다.
- 게시판과 글 이동은 기존 native 상세 경로 및 수집 글 경로 헬퍼를 재사용한다. 게시판 종류, 개수와 순서는 화면에 하드코딩하지 않는다.

### 변경 검증

- `pnpm --filter native check-types`
- `pnpm --filter native test`
- 변경 파일 Biome/Ultracite 및 `git diff --check`
- Android Studio AVD에서 수다방 전용 헤더, 검색 버튼 부재, 게시판 전체 목록 이동, 홈의 게시판 2개 묶음, 게시판별 글 최대 2개, 좌우 스냅과 가장자리 미리 보기를 확인한다.
