...
- PR 리뷰에서 지적된 신규 `text-[13px]` 2곳과 구조 변경으로 PR 추가분이 된 감지 영역 `text-[13px]` 2곳을 `text-sm`로, 공용 본문 렌더러로 옮긴 `text-[15px]` 4곳을 `text-base`로 교체해 이번 작업 소유 UI에서 임의 px 폰트 크기를 제거했다.
