...
- 드래그 중 현재 표시 크기를 `N × M`으로 보여준다. 네 모서리는 드래그 시작 시점의 종횡비를 유지하고, 좌·우 변은 너비만, 상·하 변은 높이만 변경한다.
- 최소 표시 너비와 높이는 각각 50px이고 편집 전 이미지는 가운데 정렬한다. 손잡이 조절 후에는 반대편 경계를 고정하기 위해 생긴 위치 이동을 유지한다. 원본 한 축이 50px보다 작으면 그 원본 축 크기를 최솟값으로 사용한다.
- 표시 크기는 네 모서리와 상·하·좌·우 네 변의 총 8방향 손잡이, px Slider, px 숫자 입력을 모두 제공한다.
...
- 각 item의 `displayWidthPx`, `displayHeightPx` 기본값은 `null`이며 편집 전과 같은 본문 폭 100%와 원본 종횡비를 뜻한다. 기존 저장 문서에 `displayHeightPx`가 없으면 `null`로 호환한다.
- 선택 이미지 네 모서리와 상·하·좌·우 네 변에 Pointer Events 크기 손잡이를 표시한다. 네 모서리는 드래그 시작 시점의 종횡비를 유지하고 반대 모서리를 기준으로 양 축을 함께 바꾼다. 좌·우 변은 높이를 고정한 채 너비만, 상·하 변은 너비를 고정한 채 높이만 바꾼다.
- 좌측 손잡이는 우측 경계, 우측 손잡이는 좌측 경계, 위쪽 손잡이는 아래쪽 경계, 아래쪽 손잡이는 위쪽 경계를 고정한다. 드래그로 생긴 `offsetX`, `offsetY`는 마우스를 놓아도 유지하며 편집본과 공개 상세에 동일하게 반영한다.
- 드래그를 시작할 때 실제 렌더된 너비와 높이를 px 기준값으로 잡고, 포인터 이동 거리를 축별 1px 정수 단위 변화로 환산한다.
...
- 드래그 중 dirty 판정은 Base64가 포함된 전체 문서를 매 프레임 직렬화하지 않고 별도 상태로 관리한다. 여러 이미지의 Base64 크기와 무관하게 포인터 입력을 즉시 반영해야 한다.
- 이미지 바깥의 회색·흰색 작업 캔버스는 모두 선택 해제 영역이다. 이미지, 크기 손잡이, 삽입 커서처럼 명시적인 편집 대상만 이벤트 전파를 막는다.
- 드래그 중 이미지 옆에 현재 `너비 × 높이`를 px 숫자로 표시한다.
- 너비 최솟값은 `min(50, asset.width)`, 최댓값은 원본 asset 너비이며 높이도 `min(50, asset.height)`부터 원본 asset 높이까지 제한한다. 작은 화면에서는 preview만 컨테이너에 맞춰 축소하고 저장값은 바꾸지 않는다.
- 정확한 값 입력을 위해 속성 패널에 px Slider와 px 숫자 입력을 함께 둔다.
- 방향키는 너비 1px, Shift+방향키는 10px 단위로 조절하며 현재 높이는 유지한다.
...
- 공개 화면은 `widthPx=null`이면 기존 `w-full`, 숫자면 px 너비를 사용한다. 독립 높이와 `offsetX`, `offsetY`도 반영하되 `max-w-full`로 모바일 화면보다 커지지 않게 한다.
...
- 기본 본문 폭 → 모서리 드래그 시 비율 유지, 좌우 변 드래그 시 너비만 변경, 상하 변 드래그 시 높이만 변경, 각 손잡이의 반대쪽 경계 고정과 위치 저장, `N × M` 표시, 축별 50px 최소 제한 확인.
...
- 여러 이미지가 있는 공고에서도 크기 손잡이 드래그가 지연 없이 반응하고, 회색·흰색 작업 캔버스 어디를 클릭해도 선택이 해제되는지 확인.
...
- 생성 마이그레이션: `packages/db/src/migrations/0063_lonely_famine.sql`. 이미지 편집 JSONB·마지막 작업자·마지막 작업 시각·전용 revision 컬럼과 사용자 FK만 추가된 것을 직접 확인했고 사용자가 로컬 DB에 `pnpm db:migrate`로 적용해 성공했다.
- `pnpm --filter @bambi-app/api exec vitest run src/services/bambi-crawled-image-document.test.ts`: 독립 높이와 레거시 문서 호환 검증을 포함해 4개 통과.
- `pnpm --filter web exec vitest run src/lib/bambi/crawled-image-editor.test.ts`: 모서리 비율 유지, 변 단일 축 조절, 반대 경계 고정 offset 검증을 포함해 6개 통과.
- `pnpm --filter @bambi-app/db check-types`, `pnpm --filter @bambi-app/api check-types`, `pnpm --filter web check-types`: 모두 통과.
- 이번 축별 크기 조절 변경 TypeScript·TSX 9개 파일 대상 `pnpm exec ultracite check`: 통과.
- `git diff --check`: 통과.
- 운영자 브라우저 수동 검증은 진행 중이며, 모서리 비율 유지·좌우 너비 전용·상하 높이 전용 동작은 최신 변경 반영 후 재확인해야 한다.
...
- 2026-08-05: 본 편집 화면의 표시 크기 조절도 네 모서리와 네 변을 포함한 8방향으로 확장하고, 다중 Base64 문서의 매 프레임 직렬화를 제거하며 회색·흰색 작업 캔버스 전체를 선택 해제 영역으로 정했다.
- 2026-08-05: 표시 크기 8방향 동작을 정정했다. 모서리는 드래그 시작 비율을 유지하고, 좌우 변은 너비만, 상하 변은 높이만 독립 조절하며 이를 위해 item에 하위 호환 가능한 `displayHeightPx`를 추가한다.
- 2026-08-05: 크기 조절 후에도 반대편 경계를 고정하는 첫 번째 방식을 선택했다. 기본 가운데 위치에서 발생한 축별 이동을 `offsetX`, `offsetY`로 저장해 편집 화면과 공개 상세에서 유지한다.
...
## 2026-08-05 긴 이미지 자르기 저장 오류 수정

### 원인과 수정 범위

- 높이 7,000px 중후반 이상의 JPEG를 Canvas에서 품질 92%로 다시 인코딩하면 원본보다 편집본 바이트와 Base64 요청 본문이 커질 수 있다.
- 기존 편집 문서 검증은 자산 한 장 8MB, 전체 16MB로 제한하고 서버는 Fastify 기본 요청 본문 한도를 사용한다. 이 조합에서는 큰 자르기 결과가 API 검증에 도달하기 전에 연결 단계에서 끊겨 화면에 `Failed to fetch`가 표시될 수 있다.
- 퀸알바 상세 이미지의 공통 폭 730px와 최대 높이 9,999px를 지원하고 무손실 PNG의 최악 조건도 수용하도록 편집 자산 한 장 한도를 32MB, 문서 전체 한도를 64MB로 확장한다.
- Base64가 이진 데이터보다 약 33% 커지는 점과 RPC 포장 오버헤드를 포함해 `/rpc/*` 요청 본문 한도를 96MB로 명시한다. 다른 서버 경로는 기존 기본 한도를 유지한다.
- 원본 크롤링 이미지 한도나 일반 회원 이미지 업로드 한도는 바꾸지 않는다. 확장된 한도는 운영자 수집 공고 편집 문서에만 적용한다.
- MIME·매직 바이트·실제 크기·문서 참조 무결성 검증은 그대로 유지한다.

### 검증 기준

- 서버 검증이 폭 730px, 높이 9,999px인 8MB 초과 JPEG 편집 자산을 허용한다.
- 32MB를 초과하는 단일 편집 자산과 64MB를 초과하는 편집 문서는 계속 거부한다.
- Fastify RPC 요청 본문 한도가 96MB로 고정되고 다른 경로에는 적용되지 않는다.
- 실제 폭 730px, 높이 9,999px 이미지를 자른 뒤 저장했을 때 `Failed to fetch` 없이 저장 성공 응답을 받는다.

### 구현·검증 기록

- [x] 운영자 편집 자산 한 장 한도를 32MB, 편집 문서 전체 한도를 64MB로 확장했다.
- [x] Fastify RPC 요청 본문 한도를 96MB로 명시하고 다른 경로의 기본 한도는 유지했다.
- [x] 폭 730px, 높이 9,999px, 9MB JPEG 허용과 32MB 초과 거부를 자동 테스트했다.
