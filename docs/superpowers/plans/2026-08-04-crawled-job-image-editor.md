...
- 표시 크기는 네 모서리와 상·하·좌·우 네 변의 총 8방향 손잡이, px Slider, px 숫자 입력을 모두 제공한다. 종횡비를 고정하므로 어느 손잡이를 사용해도 너비와 높이가 함께 비례 변경된다.
...
- 선택 이미지 네 모서리와 상·하·좌·우 네 변에 Pointer Events 크기 손잡이를 표시한다. 어느 손잡이를 잡아도 종횡비는 고정되고 반대편을 기준으로 크기가 변한다.
...
- 기본 본문 폭 → 모서리와 상하좌우 변 드래그 px 크기 변경, `N × M` 표시, 50px 최소 제한, 가운데 정렬 확인.
