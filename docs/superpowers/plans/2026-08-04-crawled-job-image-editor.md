...
- 최소 표시 너비와 높이는 각각 50px이고 이미지는 항상 가운데 정렬한다. 원본 한 축이 50px보다 작으면 그 원본 축 크기를 최솟값으로 사용한다.
...
- 기본 본문 폭 → 모서리 드래그 시 비율 유지, 좌우 변 드래그 시 너비만 변경, 상하 변 드래그 시 높이만 변경, `N × M` 표시, 축별 50px 최소 제한, 가운데 정렬 확인.
...
- `pnpm --filter web exec vitest run src/lib/bambi/crawled-image-editor.test.ts`: 모서리 비율 유지와 변 단일 축 조절 검증을 포함해 5개 통과.
