...
- 붙여넣기와 새 이미지 추가 위치는 운영자가 마우스로 지정해 깜빡이는 삽입 커서 자리다.
...
- 자르기 화면은 선택 영역을 밝게, 잘릴 영역을 어둡게 표시한다. 네 모서리에만 카카오톡 예시와 같은 꺾쇠형 손잡이를 두고 자유 비율로 자른다.
...
- 드래그를 시작할 때 실제 렌더된 너비를 px 기준값으로 잡고, 이동 거리를 1px 단위 너비 변화로 환산한다. 높이는 `round(width × asset.height / asset.width)`로 계산한다.
...
- 네 모서리에만 꺾쇠형 crop 표시와 pointer hit area를 둔다. 변 중앙 손잡이는 제공하지 않는다.
...
- 자르기 네 모서리 표시, 어두운 제거 영역, 밝은 유지 영역, 네 모서리 드래그와 자유 비율 확인.
...
- 2026-08-04: 운영자 전용·`active` 전용, 같은 탭 전용 페이지, 원본 별도 보존·초기화, 빈 이미지 저장, 가운데 정렬, 네 모서리 px 리사이즈·비율 고정·`N × M` 표시·50px 제한, Slider·숫자 입력, 네 모서리 자유 crop, 원본 MIME(JPEG/WebP 92%·PNG 무손실)과 GIF crop 금지, 삽입 커서, 단일 파일 추가, 모바일 점 3개, 감사 표시, 저장 후 화면 유지와 undo 초기화를 확정했다.
