...
- [x] 관련 테스트와 웹 타입 검사를 통과시킨다.
- [x] `fix: 면접 예정 목록 상태 갱신 보완` 커밋으로 분리한다.
...
- [x] 문구와 모바일 레이아웃 회귀 테스트를 먼저 추가한다.
- [x] 비회원 버튼 문구와 사용자 매뉴얼·테스트 플로우를 동기화한다.
- [x] 모바일 제목과 액션을 분리하고 제목을 최대 두 줄로 표시한다.
- [x] 관련 테스트와 웹 타입 검사를 통과시킨다.
- [x] `fix: 비회원 문구와 모바일 채팅 헤더 정리` 커밋으로 분리한다.
...
- [x] 동적 뷰포트 최대 높이와 입력란 포커스 회귀 테스트를 먼저 추가한다.
- [x] ReportDialog 최대 높이와 ReportForm textarea 스크롤 동작을 구현한다.
- [x] 관련 테스트와 웹 타입 검사를 통과시킨다.
- [x] `fix: 모바일 신고 입력창 키보드 가림 수정` 커밋으로 분리한다.
...
- [x] 관련 Vitest, `pnpm --filter web check-types`, 변경 파일 Ultracite, `git diff --check` 통과.
- [x] 이슈 본문과 PR 본문에 검증 결과를 기록하고 PR에 `Closes #85`를 넣는다.
