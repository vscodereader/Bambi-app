...
- 최신 develop `541a41fc`의 비회원 인증 재사용 계약을 반영했다. 유효한 SecureStore 게스트 토큰은 가입 화면에서 서버 상태를 다시 확인한 뒤 본인인증 단계를 건너뛰고, 만료·불완전 상태는 재인증으로, 기존 계정 충돌은 로그인으로 돌린다.
- 서버가 인증 로그를 트랜잭션에서 한 번만 소비하며, 가입 완료 뒤 게스트 토큰을 제거한다.
- 최신 develop처럼 coachmark가 활성인 동안 메인 팝업을 표시하지 않는다.
- 기존 일반 Dialog였던 coachmark를 실제 검색 버튼·채팅 탭·내 정보 탭의 native view 측정값을 사용하는 spotlight overlay로 교체했다. 고정 좌표는 사용하지 않는다.
- Android Studio AVD에서 1/3 검색 버튼, 2/3 채팅 탭, 3/3 내 정보 탭의 실제 위치가 순서대로 강조되고 메인 팝업이 동시에 뜨지 않는 것을 확인했다.
