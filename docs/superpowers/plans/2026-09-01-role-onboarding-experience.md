...
- 구직자·구인자·공통 온보딩에 공통 5장 `포인트로 실제 상품권 구매까지!`를 추가했다. 최신 `PointShopScreen`의 프리미엄 광고·포인트 아이템·2열 정사각 상품 카드·가격 배지·구직자 하단 탭 구조를 읽기 전용 코드 미리보기로 렌더링하고 저장소의 배민 1만원 상품권 원본 이미지를 사용한다. 온보딩에서는 실제 API를 호출하지 않고 `상품 1초 → 상품 클릭 → 구매 다이얼로그 2초 → 구매하기 클릭 → 마이페이지 구매 내역 3초 → 초기화`를 반복하며 필요 포인트 10,000P, 보유 포인트 50,000P, 주문완료·-10,000P·휴대폰 발송 안내·취소환불을 보여준다.
