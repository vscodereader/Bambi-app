...
3. 신규 구직자 가입 흐름은 `구직자 온보딩 5장 → 시작하기 → /seeker → 구직자 코치마크`다.
4. 신규 구인자 가입 흐름은 `구인자 온보딩 5장 → 시작하기 → /employer → 구인자 코치마크`다. 업체 정보가 없는 신규 구인자도 `/employer`로 이동하며 기존 업체 정보 등록 게이트를 재사용한다.
5. 공통 온보딩은 5장으로 만들지만 신규 가입 직후 자동으로 보여주지 않는다. 계정 설정의 `온보딩 다시 보기`에서만 연다.
...
9. 구직자·구인자·공통 온보딩은 각각 정확히 5장이다. 마지막 5장은 공통 포인트몰 안내다.
...
| 5 | `포인트로 실제 상품권 구매까지!` | `다양한 활동으로 번 포인트를 사용해 상품을 구매하세요!` | 실제 포인트몰 상품 목록 | 기존 포인트몰·상품권 아이콘 |
...
- 테스트에서 audience마다 정확히 5장, ID·이미지 경로 중복·빈 문구 없음, 마지막 CTA 계약을 검사한다.
...
- [ ] 3개 audience × 정확히 5장과 콘텐츠·이미지 경로 계약을 테스트한다.
...
| 시나리오 | 기대 결과 |
| --- | --- |
| 신규 구직자 가입 성공 | `/onboarding`, 구직자 5장, seen 키 즉시 기록 |
| 신규 구인자 가입 성공 | `/onboarding`, 구인자 5장, seen 키 즉시 기록 |
...
| 설정 역할별 다시 보기 | 5장 재생, 시작 시 역할 코치마크, seen 키 불변 |
| 설정 공통 다시 보기 | 공통 5장, 역할 메인 이동, 코치마크 없음 |
...
- 구직자·구인자·공통 온보딩에 공통 5장 `포인트로 실제 상품권 구매까지!`를 추가했다. 최신 `PointShopScreen`의 프리미엄 광고·포인트 아이템·2열 정사각 상품 카드·가격 배지·구직자 하단 탭 구조를 읽기 전용 코드 미리보기로 렌더링하고 저장소의 배민 5만원 상품권 원본 이미지를 사용한다. localhost DB에는 상품권 카테고리·쿠폰 발송형·전체 회원·무제한 재고·50,000P·노출 활성 상품을 실제 생성해 `/point-shop`에서 이미지와 카드 표시를 확인했다. 온보딩에서는 실제 API를 호출하지 않고 `상품 1초 → 상품 클릭 → 구매 다이얼로그 2초 → 구매하기 클릭 → 마이페이지 구매 내역 3초 → 초기화`를 반복하며 주문완료·-50,000P·휴대폰 발송 안내·취소환불을 보여준다.
- 운영자 포인트몰 이미지 업로드는 로컬 인텐트의 상대 경로를 그대로 API에 보내 절대 URL zod 검증에서 실패하던 문제를 수정했다. 개발 환경에서만 `window.location.origin`으로 절대 localhost URL을 만들고 production GCS 경로는 기존대로 유지한다.
