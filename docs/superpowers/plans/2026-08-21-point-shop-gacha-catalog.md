...
- 화면과 보상은 동일한 현재 연속 출석을 사용한다. 오늘 또는 어제부터 거꾸로 하루도 비지 않은 전체 출석 구간이며 기능 배포 전 기존 출석·일반 출석·복구 출석을 구분하지 않는다.
- 현재 연속 출석이 7·14·21일처럼 7일 단위를 완성할 때마다 뽑기권 1장을 지급한다. 화면이 7일인데 보상 대상은 6일처럼 서로 다른 수치를 만들지 않는다.
- 정책 변경 시 이미 현재 연속 7일 이상이지만 기존 `streak_reward_eligible` 필터로 보상을 받지 못한 구직자에게 migration에서 누락 claim과 뽑기권을 즉시 지급한다. 기존 claim 수와 원장 external key로 중복 지급을 막는다.
...
- 모든 활성 당첨 항목은 포인트가 1 이상이고 최종 계산된 당첨 확률이 0%보다 커야 한다.
- 운영자 화면에는 가중치 개념을 노출하지 않는다. 운영자는 각 항목의 `당첨 확률(%)`에 숫자를 직접 입력하거나 비워 자동 계산을 선택한다.
- `0.0001`을 입력하면 해당 항목은 실제 추첨에서 정확히 `0.0001%` 확률을 가진다. 입력·표시·API·DB·난수 추첨은 백분율의 소수점 아래 6자리까지 지원한다.
- 확률을 직접 입력한 활성 항목은 입력값을 그대로 고정한다. 확률을 비운 활성 항목은 `100% - 직접 입력한 활성 항목의 합계`를 자동 항목 수로 균등 분배한다.
- 자동 분배가 최소 확률 단위로 나누어떨어지지 않으면 정렬상 마지막 자동 항목이 나머지를 받아 활성 항목 합계를 정확히 100%로 맞춘다.
- 비활성 항목은 직접 입력값 보존 여부와 관계없이 합계와 추첨에서 제외한다. 다시 활성화하면 현재 활성 항목 전체를 기준으로 자동 확률을 재계산한다.
- 직접 입력한 활성 확률 합계가 100% 이상이거나, 자동 항목 없이 합계가 정확히 100%가 아니거나, 남은 확률이 자동 항목 모두에게 최소 단위보다 크게 배분될 수 없으면 운영자 저장과 사용자 뽑기를 거부하고 현재 합계·남은 확률을 안내한다.
- 운영자 화면은 각 항목의 `직접 입력` 또는 `자동 계산` 상태와 실제 적용 확률, 활성 항목의 직접 입력 합계와 전체 적용 합계를 보여준다.
- 운영자의 당첨 포인트·직접 확률 입력, 활성 토글, 항목 추가·삭제는 즉시 서버에 반영하지 않고 화면 로컬 초안에만 반영한다. 실제 적용 확률 미리보기만 초안 기준으로 즉시 다시 계산한다.
- 하단 `당첨 설정 저장` 버튼을 눌렀을 때 초안 전체를 한 번의 서버 요청과 DB 트랜잭션으로 저장한다. 저장 전 새로고침은 마지막 서버 저장 상태로 돌아가며, 변경 없음·입력 오류·확률 합계 오류·저장 중에는 저장 버튼을 비활성화한다.
- 서버는 일괄 저장 요청의 기존 ID 존재·중복, 포인트/확률 범위, 활성 항목 자동 분배 가능 여부를 먼저 검증한 뒤 수정·추가·삭제·정렬을 원자적으로 반영한다. 중간 실패 시 일부 행만 저장하지 않는다.
- 서버는 클라이언트가 계산한 예상값을 신뢰하지 않고 저장된 직접 확률과 활성 항목을 다시 읽어 동일한 자동 분배를 계산한 뒤, 0부터 100% 고정 정수 범위의 암호학적 난수로 당첨 항목을 선택한다.
- 기존 정수 가중치 설정은 Drizzle migration에서 당시 활성 가중치 비율을 소수점 아래 6자리 백분율 정수 단위로 변환한다. 반올림 잔여분은 정렬상 마지막 활성 항목에 배정해 합계를 정확히 100%로 유지하며, 비활성 항목은 자동 계산 상태로 이관한다.
- 기존 `prize_weight_snapshot`은 과거 감사 이력을 위해 보존한다. 신규 추첨에는 적용 확률 스냅샷을 별도 기록하고 더 이상 가중치를 추첨 계약으로 사용하지 않는다.
...
- 인기상품 후보는 선택 여부와 무관하게 공용 `FeaturedItemRow`로 렌더한다. 미선택 행도 기존 `icon-sm` 이동 버튼 영역을 `invisible`로 보존해 임의 높이값 없이 선택·미선택 행의 크기를 동일하게 유지한다.
...
- 운영자 상품 폼에는 내부 `benefit_type` 선택을 노출하지 않는다. 일반 신규 상품은 기존 수동 지급형을 재사용하고, 시스템 기능 상품은 migration에서 연결한 내부 유형을 수정 없이 유지한다.
- 상품 폼의 분류 선택 라벨은 `노출 카테고리`이며 `포인트몰 위치 설정`에 배치된 활성 standard 분류를 그대로 사용한다.
- `노출 카테고리` 아래에는 운영용 `상품 유형`을 `일반 상품`, `포인트권`, `끌어올리기권`, `상품권` 네 가지로 표시한다. 이는 진열 위치가 아니라 구매 후 처리 방식의 사용자용 묶음이다.
- `포인트권`에는 포인트로 구매한 뒤 사용하는 뽑기권과 출석 복구권이 포함된다. 기존 시스템 상품의 세부 내부 유형은 유지한다.
- 운영자 상품 폼에서 `포인트권`을 선택하면 기존 수량형 혜택 카탈로그를 재사용해 `뽑기권`과 `출석복구권` 하위 유형을 반드시 선택·확인할 수 있게 한다. 신규 포인트권을 이름으로 추론하거나 무조건 뽑기권으로 저장하지 않는다.
- `끌어올리기권`을 선택했을 때만 `수동`, `자동`, `횟수권` 단일 선택을 표시한다. 수동·자동은 하루 끌어올리기 횟수와 기간을, 횟수권은 총 횟수를 입력한다.
- `일반 상품`은 구체 이행 정책이 확정되지 않았으므로 기존 수동 처리 흐름만 유지한다. `상품권` 초기 상품은 만들지 않는다.
- 초기 노출 카테고리는 `상품권`, `끌어올리기`, `출석`, `기타`이며 모든 카테고리의 상품 목록은 비워 둔다.
- migration이나 애플리케이션 코드에서 초기 상품을 생성하지 않는다. 모든 상품은 운영자가 상품 추가 화면에서 직접 등록한다.
- 상품 가격은 초안에서 null을 허용한다. 공개 상품은 가격이 반드시 입력되어야 하며 구매·정렬은 가격이 있는 공개 상품만 대상으로 한다.
- 재고 입력을 비우면 0으로 저장한다. `무제한` 체크박스를 선택한 경우에만 `stock_quantity=null`을 저장한다.
- `행 추가`는 이름 입력 Dialog를 열고 저장 시 새 standard 분류와 마지막 배치 행을 한 트랜잭션으로 만든다. 별도 빈 행·별도 분류 생성 UI는 두지 않는다.
- standard 행 이름은 대소문자를 구분하지 않고 고유해야 하며 생성·이름 수정 모두 API와 DB 고유 인덱스에서 중복을 거부한다.
- 빈 standard 분류 삭제 시 연결된 배치 행도 함께 삭제하고 뒤 행 position을 조밀하게 재정렬한다. 상품이 하나라도 속한 분류 삭제는 서버와 UI에서 막는다.
- 이번 단계의 상품 유형은 실행 동작과 분리된 이름 분류다. 운영자는 포인트몰 설정 `상품 유형 설정` Accordion에서 유형명을 추가·삭제하고, 저장된 유형은 상품 추가·수정 폼의 `상품 유형` Select에 즉시 표시한다.
- 새 유형 입력 영역 아래에 기존 유형 목록을 표시하고 각 행에 편집·삭제 버튼을 둔다. 편집 버튼은 후속 설정을 추가할 수 있는 빈 편집 영역을 행 아래에 펼치며 이번 단계에서는 추가 필드를 저장하지 않는다.
- 기존 유형 각각을 실제 중첩 AccordionItem으로 렌더해 유형명 행을 누르면 편집 영역이 펼쳐지고, 별도 편집 버튼은 두지 않는다.
- 상품 유형은 DB에 이름과 생성 시각만 저장하며 코드 배열·상품명·UUID를 하드코딩하지 않는다. 상품은 선택적으로 유형을 참조하고 기존 상품은 null 상태를 유지한다.
- 기존 고정 실행 동작 UI는 `구매 후 실행 동작`으로 유지한다. 유형별 추가 내용·동작 연결·대상·순서 정책은 후속 요구사항에서 확장한다.
- 초기 migration은 기존 상품 폼에 이미 존재하던 `일반 상품·포인트권·끌어올리기권·상품권` 네 유형을 DB 행으로 이관해 설정 Accordion과 상품 폼에서 바로 보이게 한다. 이후 목록 표시는 DB만 정본으로 사용한다.
- 상품 유형 편집 영역은 유형명과 `내 아이템에 표시` 여부를 저장한다. 아이템 폼의 중복된 기존 유형 UI는 `구매 후 실행 동작` 단일 Select로 정리하고 뽑기권·복구권 라디오를 제거한다. 잘못 저장된 역느낌표는 일반 느낌표로 교정한다.
- 상품 유형 저장 성공 시 편집 영역을 자동으로 닫지 않고 성공 토스트를 표시한 채 열린 상태를 유지한다. 사용자가 편집 버튼을 다시 눌렀을 때만 접는다.
- 유형 목록은 생성 시각과 ID의 안정 정렬을 사용해 저장 후 재조회해도 행 순서와 열린 편집 영역 위치가 바뀌지 않게 한다.
...
- 뽑기권 `사용하기`는 버튼 스타일로 렌더되는 Next.js 링크이므로 앵커 스타일 우선순위까지 덮는 저장소의 기존 `no-underline!` 링크 버튼 관례를 적용해 브라우저 기본 밑줄을 제거한다.
...
- `streak_reward_eligible`은 과거 이력 호환을 위해 컬럼을 유지하지만 정책 변경 migration에서 기존 행도 true로 통일하고 보상 계산에서는 더 이상 필터로 사용하지 않는다.
...
- 출석 날짜 하나가 추가될 때 그 날짜를 포함하는 전체 출석 연속 구간을 계산한다. 화면의 현재 연속 출석과 같은 날짜 집합을 사용한다.
...
| `id` | uuid PK | |
| `points` | integer | 1 이상 |
| `probability_units` | integer nullable | 직접 입력한 백분율의 소수점 6자리 정수 단위, null은 자동 계산 |
...
- 운영자 화면에는 직접 입력값과 자동 계산된 실제 적용 확률을 표시하며 서버도 같은 공용 계산 함수를 사용한다.
...
| `prize_weight_snapshot` | integer nullable | migration 이전 과거 감사 이력 보존 전용 |
| `prize_probability_units_snapshot` | integer nullable | 신규 추첨의 실제 적용 확률 스냅샷 |
...
- 활성 prize를 운영자 정렬 순서와 ID 순서로 안정적으로 읽는다.
- 직접 입력 확률을 고정하고 나머지를 자동 항목에 균등 분배하는 순수 함수를 UI와 서버가 재사용한다.
- 누적 확률 정수 단위 선택 순수 함수는 경계값 테스트가 가능하도록 RNG 값을 인자로 받는다.
...
- `포인트 상품 추가` 버튼.
- 사진 업로드, 이름, 설명, 노출 카테고리, 가격, 대상 역할, 상품별 설정, 재고·무제한, 공개 토글을 입력한다.
- 내부 혜택 유형은 운영자 폼에서 직접 노출하지 않고, 운영자가 선택한 상품 유형과 끌어올리기 종류에 따라 기존 서버 유형으로 연결한다.
...
- 각 행에 당첨 포인트, 직접 입력 가능한 `당첨 확률(%)`, 현재 실제 적용 확률을 표시한다.
- 당첨 포인트와 당첨 확률 필드는 동일한 `flex flex-col gap-2` 구조를 재사용해 두 라벨과 입력창 사이를 충분히 분리하고 같은 행 높이·위치로 정렬한다. 두 라벨은 입력 컴포넌트의 기존 내부 가로 여백과 같은 `px-3` 토큰을 사용해 아래 숫자 시작선과 맞춘다.
- 확률 입력을 비우면 자동 계산 상태이며 직접 입력 합계와 자동 계산 후 전체 100% 상태를 하단에 표시한다.
- 입력·활성 토글·추가·삭제는 모두 저장 전 초안이며 실제 확률 라벨만 즉시 갱신한다. 초안에 유효한 변경이 있을 때만 하단 `당첨 설정 저장` 버튼을 활성화한다.
...
- 확률 문자열은 소수점 6자리까지 정수 단위로 변환하고 0% 초과 100% 이하 범위와 활성 합계·자동 분배 가능 여부를 서버에서 다시 검증한다.
...
- 직접 확률 selector 경계, `0.0001%` 보존, 자동 균등 분배·나머지 보정, 비활성 prize 제외, 설정 없음 거부
...
- 기존 출석·일반 출석·복구 출석이 함께 이어진 현재 연속 7일에서 즉시 지급되고 화면 수치와 보상 수치가 일치하는지 검증
...
- [x] Task 5 운영자 직접 확률·자동 균등 분배 기반 랜덤 뽑기 서버, 멱등 요청, 실제 적립액 기록
...
- `0112_thin_pandemic`: 최신 develop의 0111 뒤로 기존 포인트몰 0109·0110 SQL을 통합 승격했다. 핵심 테이블·enum·분류 seed·기존 상품 other 이관·배포 전 출석 보상 제외와 `member_item_transaction` 알림 target을 포함한다.
- 기존 migration 워터마크가 `0098~0100`을 건너뛴 DB에서는 누락된 포인트몰 기반을 조건부 복구한 뒤 통합 0112를 적용한다.
- 실제 개발 DB는 Drizzle 원장이 `0109_yielding_songbird`에서 멈췄지만 0112 대상 enum 5개·핵심 테이블 8개·출석/상품 확장 컬럼은 이미 존재하는 이력 불일치 상태다. `0112_thin_pandemic`의 enum·테이블·컬럼·constraint·index·seed를 재실행 가능하게 만들어 기존 객체와 데이터를 삭제하지 않고 원장을 정상 전진시킨다.
- enum은 duplicate_object 예외 또는 `ADD VALUE IF NOT EXISTS`, 테이블·컬럼·index는 `IF NOT EXISTS`, constraint는 `duplicate_object` 예외, seed는 `ON CONFLICT DO NOTHING`으로 보호한다. 빈 DB에서는 동일 최종 schema를 만들고, 부분 적용 DB에서는 없는 객체만 보충한다.
- 수정 후 개발 DB 상태 위에서 0110~0114 롤백 시뮬레이션과 빈 PostgreSQL 18의 0000~0114 전체 migrate를 각각 통과했다. 실제 개발 DB에도 `db:migrate`를 적용해 Drizzle 원장을 0114까지 전진시켰고 확률 컬럼·과거 snapshot nullable·활성 확률 합계 100,000,000 단위를 직접 확인했다.
- `db:push` 미사용
- `drizzle-kit check`: `Everything's fine`
- 재 `drizzle-kit generate`: `No schema changes`
- 최신 develop 기준 새 DB에서 `db:migrate`: 0109~0113 전체 적용 성공
- migration 원장 2건과 포인트몰·아이템 원장·뽑기·후기 보상 테이블, 초기 분류·신규 benefit enum 직접 조회 확인
- `0113_lazy_moonstone`: 상품 가격 nullable 초안, 중복 분류명 고유 인덱스, 끌어올리기·출석 카테고리와 동적 행 정리를 추가하며 상품 데이터는 생성하지 않는다.
- 로컬 DB에서 0113 실제 적용 후 인기상품 → 상품권 → 끌어올리기 → 출석 → 기타 위치와 빈 상품 목록을 직접 조회했다.
- `0114_freezing_ulik`: 당첨표의 정수 가중치를 직접 확률 정수 단위로 전환하고 기존 활성 가중치 비율을 합계 100%가 유지되도록 변환한다. 과거 weight snapshot은 nullable 감사 이력으로 보존하고 신규 확률 snapshot을 추가한다.
- 후속 데이터 교정 migration은 이름·UUID를 하드코딩하지 않고 `attendance` 카테고리에 연결됐으나 `draw_ticket`으로 저장된 기존 상품을 `attendance_restore_ticket`으로 전환한다. 해당 item의 기존 주문·연결 아이템 원장 유형도 함께 전환하고, 영향 사용자의 두 수량형 아이템 `balance_after`를 시간순 원장 합계로 재계산한다.
- `0115_repair-attendance-restore-ticket-ledger`: 개발 DB 롤백 시뮬레이션과 빈 PostgreSQL 전체 migrate를 통과한 뒤 실제 적용했다. 기존 주문·원장 5건을 복구권으로 전환하고 뽑기권 0장·복구권 5장, 음수 balance 0건을 확인했으며 구직자 실제 `내 아이템`에서 `출석복구권 · 보유 5개 · 사용하기` 렌더를 검수했다.
- `0116_unify-attendance-streak-reward-policy`: 화면과 보상을 전체 출석 기준 현재 연속 구간으로 통일하고 기존 제외 행을 보상 대상으로 전환했다. 개발 DB 롤백 시뮬레이션·빈 DB 전체 migrate 후 실제 적용해 누락 claim 1건과 뽑기권 1장을 지급했으며, 실제 화면에서 `연속 출석 7일 · 다음 뽑기권까지 7일`, `뽑기권 보유 1개`, `출석복구권 보유 1개`를 확인했다.
- `0117_wild_timeslip`: 이름만 관리하는 상품 유형 테이블과 상품의 nullable 유형 FK를 추가했다. 포인트몰 설정 Accordion에서 유형을 추가·삭제하고 상품 폼 Select가 같은 API 데이터를 사용하며, 구매 후 실행 동작은 기존 로직과 분리해 유지한다.
- 임시 PostgreSQL 18 새 DB에서 0000~0114 전체 `db:migrate` 성공, 신규 확률 컬럼·nullable 과거 snapshot을 직접 조회했다.
- 별도 임시 backfill DB의 기존 80/20/1 활성 가중치는 79.207920%/19.801980%/0.990100%로 변환되어 합계 100%를 유지했고 비활성 행은 자동(null), 과거 draw snapshot은 보존됐다.
...
- 직접 확률 서비스 테스트 6건 통과: `0.0001%` 파싱·표시, 자동 균등 분배, 마지막 항목 나머지 보정, 누적 경계, 범위·합계 오류
- 운영자 당첨 설정은 로컬 초안과 `adminSavePrizes` 일괄 저장으로 전환했다. 입력·토글·추가·삭제 핸들러에는 즉시 mutation이 없고 저장 버튼만 서버 mutation을 호출하는 것을 소스 검사로 확인했다.
- 당첨 포인트·확률 라벨은 공통 `px-3`, 필드 스택은 공통 `gap-2`를 사용해 임의 px 없이 숫자 시작선과 행 위치를 통일했다.
- DB/API/Server/Web 타입 검사, 변경 파일 Biome, `drizzle-kit check`, 재 generate No schema changes, `git diff --check` 통과.
- 전체 API 회귀 테스트는 현재 develop의 커뮤니티 surface 계약 변경과 기존 테스트 입력 불일치(`surface` 누락) 등 범위 밖 실패가 다수 발생해 중단했다. 직접 확률 관련 테스트는 독립 실행으로 통과했다.
- 실제 운영자 UI 검수는 현재 브라우저 세션이 구직자 역할이라 `/moderator/point-shop` 접근 시 `/seeker`로 리다이렉트되어 수행하지 못했다. 운영자 세션에서 데스크톱·375×667 검수가 남아 있다.
...
- 참고 이미지 기준 상단 캡·유리통·캡슐볼 14개·사다리꼴 본체·크랭크·출구·받침 재구성, 320ms 간격에서 14개 볼 전부 위치 변경 실측
...
- 포인트몰 서비스 테스트 47건, API/Web/DB 타입 검사, 변경 파일 Ultracite, `drizzle-kit check`를 통과했다.
- 운영자 브라우저에서 노출 카테고리 → 상품 유형 4종, 끌어올리기 수동·자동·횟수권 전환과 조건부 수치 입력, 재고 0·무제한 체크, 행 추가 Dialog, 상품 보유 분류 삭제 차단을 확인했다.
