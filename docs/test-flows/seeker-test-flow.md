...
- 데스크톱 헤더와 모바일 헤더의 해/달 버튼으로 라이트·다크를 왕복한다.
- 최초 방문은 라이트여야 하고, 수동으로 고른 테마는 새로고침·직접 URL 이동·재방문 뒤에도 유지되어야 한다. 시스템 테마는 자동 추종하지 않는다.
- 다크모드에서 페이지는 `ink-950`, 카드·팝오버는 `ink-900`/`ink-800` 계열로 층위가 나뉘고 일반 제목·본문은 밝은 문자여야 한다.
- 브랜드·상태 버튼과 기존 테두리 색은 유지한다. outline 버튼은 다크모드에서 카드 배경·밝은 글자로 바뀌고, 로그아웃과 실제 메인 팝업만 흰 배경·어두운 글자를 유지해야 한다.
- `/jobs/**`, `/board/**`, `/seeker/**`, 로그인·회원가입, `/support/**`, 법적 문서·매뉴얼·포인트몰과 모바일 채팅 상세 헤더를 확인한다.
- 업로드 이미지·GIF·광고 소재와 편집 결과에는 반전·필터가 적용되지 않아야 한다.
- 메인 팝업은 다크모드에서도 기존 라이트 화면을 유지하고, 팝업을 닫은 주변 페이지는 선택한 다크모드를 유지해야 한다.
- 내부 디자인 데모 `/preview`는 적용·검수 대상에서 제외한다.
...
### 모바일 회원가입 본인인증 복귀 회귀 테스트

- **절차**: 모바일 Chrome에서 `/seeker?auth=signup` 진입 → 본인인증 완료 → 리디렉션으로 서비스 복귀.
- **기대 결과**: 복귀 URL에서 포트원 결과 파라미터만 제거되고 `auth=signup`은 유지된다. 로그인 폼을 거치지 않고 회원가입 2단계 폼이 즉시 표시되어야 한다.
- **회귀 확인**: 데스크톱 팝업 인증 흐름과 `auth=login` 로그인 화면은 기존과 동일해야 한다.
...
- **기대 결과**:
  ...
  - 첫 48건 후보 중 수집 공고가 삭제됨(`removed`)으로 바뀌면 그 행은 제외하고 다음 노출 가능 공고를 당겨 다시 48건을 채운다. 더보기 묶음도 같은 규칙이며 전체 잔여 공고가 부족한 마지막 묶음만 48건 미만이다
...
- **읽음 처리**: 방 진입 후 메시지 조회가 성공하는 즉시 상대 발신 메시지 id를 모아 `bambi.chats.markRead`를 호출한다. 메시지를 보내지 않아도 성공 직후 현재 방 목록과 전역 하단 채팅 핀을 0으로 동기화한다. **입력 제약 `messageIds` 1~50개**
- **엣지 케이스**:
  - id가 UUID가 아니면 렌더 없이 `/seeker/chats`로 replace
  - 읽음 실패 시 하단 "읽음 상태를 반영하지 못했어요."
  - 말풍선에 읽음 표시(1/읽음) UI는 **없음** — 읽음은 목록의 미확인 카운트에만 반영
  - 상대 메시지 1건 수신 → 방 진입만 하고 발신하지 않음 → 목록 복귀와 즉시·30초 후 새로고침 모두 방 배지와 하단 채팅 핀이 0이어야 함
  - 읽음 성공 뒤 지연 도착한 실시간/폴링 응답이 이전 값 `1`을 다시 덮어쓰면 회귀
  - 모바일에서 긴 공고 제목은 작은 글씨의 한 줄 말줄임으로 표시하고, 공고 보기·상태·실시간 배지는 제목 아래 별도 행에 유지
  - Android 키보드가 열리면 채팅 하단 내비게이션을 숨기고 입력창은 키보드 바로 위에 유지한다. 메시지 영역만 가용 높이에 맞게 줄어들며 내부 스크롤이 가능해야 함
...
- **기대 결과**: `report.status='open'`으로 접수. 완료 화면 "신고가 접수됐어요 / 운영팀이 대화 내용을 검토하고 24시간 내 조치해요. 안전을 위해 해당 채팅은 잠시 숨겨둘게요." 채팅방 또는 메시지 신고는 신고자와 상대방 양쪽 목록·안 읽음 집계에서 즉시 숨겨진다.
- **엣지 케이스 / 실패 케이스**:
  | 상황 | 결과 |
  |---|---|
...
- **양쪽 차단 회귀 테스트**:
  1. 구직자가 채팅방 또는 채팅 메시지를 신고한다.
  2. 구직자·구인자 양쪽에서 목록 미노출, 직접 URL 열람 실패, 텍스트·첨부·면접·연락처·읽음·타이핑 전송 실패를 확인한다.
  3. 숨겨진 방의 새 메시지가 하단 채팅 핀을 만들지 않는지 확인한다.
  4. 신고 상태 `resolved`는 계속 숨김, `dismissed`는 양쪽 목록과 사용 권한을 즉시 복구해야 한다.
- **신고 취소·삭제**: **프로시저 없음**
