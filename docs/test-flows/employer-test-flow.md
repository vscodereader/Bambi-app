...
## 2026-08-18 이용정지 공고 등록 회귀 검증

## 2026-09-01 신규 구인자 온보딩 회귀 검증

1. 성인인증을 끝낸 신규 구인자로 가입하면 일반 로그인과 달리 `/onboarding`으로 이동한다.
2. 구인자 콘텐츠가 정확히 4장이고 좌우 스와이프, 이전·다음, 페이지 상태 점이 동작한다.
3. 첫 화면이 표시된 즉시 사용자 ID·구인자 역할별 완료 키가 저장된다. 첫 화면에서 브라우저를 닫고 다시 로그인해도 자동 재노출되지 않는다.
4. `건너뛰기`는 `/employer`로 이동하고 코치마크를 열지 않는다.
5. 마지막 `시작하기`는 `/employer`로 이동하고 업체 정보·공고 등록·지원자 채팅 코치마크를 한 번만 연다. 새로고침하면 다시 열리지 않는다.
6. 미승인 구인자는 기존 업체 정보 등록 게이트를 유지하며, 모바일·데스크톱 header의 채팅 진입점은 기존 `/seeker/chats`와 안 읽음 배지를 재사용한다.
7. 계정 설정의 구인자·공통 이용 안내 다시 보기는 완료 키를 변경하지 않는다.

- 이용정지 구인자는 내 공고·성과 분석의 `새 공고 등록`, 데스크톱 상단 `공고 등록`, 모바일 하단 `공고 등록` 진입점이 모두 비활성화된다.
- 이용정지 구인자가 `/employer/new`를 직접 열면 입력 폼이나 영어 서버 오류 대신 한국어 이용정지 안내가 표시된다.
- 정상·경고 구인자의 기존 공고 등록 흐름과 서버의 이용정지 최종 거부는 유지된다.
- 이용정지 구인자도 우측 하단 `1:1 상담(운영자 문의)`의 기존 대화 조회·새 문의 생성·메시지 전송은 가능하며, 운영자가 해당 문의자를 별도로 차단한 경우만 기존 정책대로 전송이 제한된다.
- 공용 숫자 입력 페이지네이션에는 해당 목록의 `/ 총 페이지 수`가 표시된다.
