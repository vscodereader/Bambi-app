...
- 데스크톱·모바일 헤더의 해/달 버튼으로 테마를 전환하고 새로고침·재방문 뒤에도 선택이 유지되는지 확인한다.
- `/employer/**`의 내 공고·공고 등록/수정·업체 정보·조직 설정·광고 안내·프로모션·분석·출석과 `/ad-banner-editor` 독립 창을 확인한다.
- 다크모드에서는 일반 흰 표면과 어두운 글자를 어두운 계층 표면·밝은 글자로 전환한다. 브랜드/상태 버튼과 기존 테두리 색은 유지하고, outline 버튼은 카드 배경·밝은 글자를 사용하되 로그아웃만 라이트 outline을 유지해야 한다.
- 공고·배너 업로드 이미지와 광고 배너 편집 캔버스 결과에는 반전·필터가 적용되지 않고 주변 패널·도구 UI만 다크모드여야 한다.
- 메인 팝업은 다크모드에서도 라이트 화면을 유지해야 하며 내부 디자인 데모 `/preview`는 대상에서 제외한다.
