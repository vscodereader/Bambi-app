...
- **기대 결과**: 제목·발췌(목록 셀은 15자 절단, 서버 발췌 120자)·작성자·상태·작성일.
  ...
  - **게시판 열**은 커뮤니티 글 탭에만 붙는다(선택 열과 같은 조건 — 펼친 행 colSpan은 `+2`).
...
- **QA 시나리오(게시판 비우기 → 게시판 삭제)**:
  1. 새 게시판 생성(8.8) → 글 1건 작성 → 게시판 삭제 시도 → 409 "글이 있는 게시판은 삭제할 수 없습니다…".
  2. 게시물 조치에서 게시판 필터로 그 게시판 선택 → 해당 글 **삭제** 조치(사유 2자 이상).
  3. 게시판 삭제 재시도 → **여전히 409**(소프트 삭제라 행이 남아 있다 — D5 소견의 재현 지점).
  4. 같은 행에서 **영구 삭제하기** → 확인 → 목록에서 사라짐.
...
- **절차**: 상태 필터(전체/수집됨/업종 검토 대기/만료/삭제됨) → 행 `…` → **삭제**(AlertDialog 확인) /
  **복구**(확인창 없이 즉시).
...
- **관련 API**: `bambi.crawler.removePost` / `restorePost` (`crawler.ts` L198 / L211)
