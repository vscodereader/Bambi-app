...
- 데스크톱·모바일 헤더의 해/달 버튼으로 테마를 전환하고 새로고침·직접 URL 이동·재방문 뒤에도 선택이 유지되는지 확인한다.
- `/moderator/**`의 목록·상세·테이블·필터·Dialog·Sheet·Dropdown·Toast와 수집 이미지·광고 상품 편집 UI를 확인한다.
- 다크모드에서는 페이지·카드·팝오버와 일반 글자를 전환하고 브랜드/상태 버튼과 기존 테두리 색은 유지한다. outline 버튼은 카드 배경·밝은 글자를 사용하되 로그아웃만 라이트 outline을 유지한다.
- 이미지·GIF·광고 소재·수집 이미지 편집 결과에는 반전·필터가 적용되지 않아야 한다.
- `/moderator/popups` 관리 화면과 그 Dialog·Popover는 운영자 콘솔의 선택 테마를 따르고, 실제 사용자에게 노출되는 메인 팝업만 기존 라이트 화면을 유지해야 한다.
