...
// 빈 문자열로 읽는다 — 형제 화면들과 같이 화면이 스스로 제목을 주입한다. 로딩·에러
// 조기 반환에서도 제목이 유지되게 모든 분기에서 함께 렌더한다.
...
export default function SeekerAccountSettingsScreen() {
	...
	const confirmWithdraw = () => {
		Alert.alert(
			"정말 탈퇴하시겠어요?",
			"탈퇴 즉시 모든 기기에서 로그아웃되고 스스로 되돌릴 수 없어요. 남긴 채팅·리뷰는 '탈퇴한 회원'으로 표시되고, 포인트로 구매한 보유 아이템·쿠폰도 함께 사라져요.",
			[
	...
	if (!profile) {
		return (
			<BambiScreen>
				<Stack.Screen options={SCREEN_OPTIONS} />
				<StateCard
					description="프로필 설정을 먼저 마친 뒤 다시 시도해 주세요."
					title="프로필 정보를 찾을 수 없어요"
				/>
			</BambiScreen>
		);
	}
	...
	return (
		<BambiScreen>
			...
			<Surface className="gap-3 rounded-lg p-4" variant="secondary">
				...
				<InfoRow label="생년월일" value={formatBirthDate8(profile.birthDate)} />
			</Surface>
			...
			{/* 위험 구역. 표시 이름 저장이 이 화면의 primary라 탈퇴 버튼은 danger로 둔다. */}
			<Surface className="gap-3 rounded-lg p-4" variant="secondary">
				<Text className="font-semibold text-base text-foreground">
					회원 탈퇴
				</Text>
				<Text className="text-muted text-sm">
					...
					회원'으로 표시되고, 아이디·이메일·연락처 등 개인정보는 {retentionDays}
					일 동안 보관한 뒤 파기돼요. 그동안에는 같은 명의로 본인인증을 다시 할
					수 없어요.
				</Text>
