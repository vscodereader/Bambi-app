...
import {
	...
	LegalSubheading,
	LegalTable,
} from "@/src/components/legal-doc";
import {
	PrivacyContactLine,
	PrivacyProcessorsTable,
	PrivacyRetentionDays,
} from "@/src/components/privacy-contacts";

// (seeker)/_layout.tsx를 건드리지 않으려고 화면이 스스로 헤더 타이틀을 주입한다.
const SCREEN_OPTIONS = { title: "개인정보 처리방침" } as const;

export default function SeekerPrivacyScreen() {
	return (
		<>
			...
			<BambiScreen>
				<LegalDoc
					...
					intro={
						<>
							<LegalParagraph>
								밤비알바({BAMBI_COMPANY.url} 이하 "회사"라 함)는 이용자의
								개인정보를 중요하게 생각하며, 이용자가 회사의 서비스(이하
								"밤비알바 서비스" 또는 "밤비알바"라 함)를 이용하면서 제공한
								개인정보가 보호받을 수 있도록 최선을 다하고 있습니다.
							</LegalParagraph>
							<LegalParagraph>
								회사는 「개인정보 보호법」 및 「정보통신망 이용촉진 및 정보보호
								등에 관한 법률」 등 관계 법령과 개인정보보호위원회가 정한 지침을
								준수하고 있습니다.
							</LegalParagraph>
							<LegalParagraph>
								회사는 이 개인정보 처리방침을 통하여 이용자가 제공하는
								개인정보가 어떠한 목적과 방식으로 처리되고 있으며 개인정보
								보호를 위해 어떠한 조치가 취해지고 있는지 알려드립니다. 이
								처리방침은 홈페이지 하단에서 언제든지 확인할 수 있습니다.
							</LegalParagraph>
							<LegalParagraph>
								밤비알바는 만 19세 이상만 이용할 수 있는 성인 대상 서비스입니다.
								회사는 휴대폰 본인확인을 통해 만 19세 미만의 가입과 이용을
								차단하고 있으며, 만 14세 미만 아동의 개인정보를 수집하지
								않습니다.
							</LegalParagraph>
						</>
					}
				...
				>
					<LegalSection heading="1. 수집하는 개인정보의 항목과 수집 방법">
						...
						<LegalParagraph>
							업소회원이 채용공고를 등록하려면 위 정보를 제출해 회사의 심사를
							거쳐야 합니다.
						</LegalParagraph>

						<LegalSubheading>
							다. 성인 인증(만 19세 이상 확인) 시
						</LegalSubheading>
						<LegalParagraph>
						...
						<LegalParagraph>
							- 회원 데이터베이스에 저장하는 정보 : 검증된 이름, 휴대폰번호,
							성별, 생년월일과, 연계정보(CI)·중복가입확인정보(DI)를 복호화할 수
							없도록 일방향 암호화(SHA-256 해시)한 값
						</LegalParagraph>
						<LegalParagraph>
							- 저장하지 않는 정보 : 통신사 정보,
						...
						<LegalParagraph>
							비회원(게스트)으로 인증하는 경우 비밀글 작성과 신고·분쟁 대응을
							위해 검증된 이름·휴대폰번호·성별을 인증일로부터 30일 동안 서버에
							분리 보관합니다. 브라우저의 밤비알바 게스트 쿠키(bambi_guest,
							유효기간 30일)에는 실명과 전화번호를 넣지 않고 서명된
							식별자·성별·만료 시각만 보관합니다.
						</LegalParagraph>
						...
						<LegalParagraph>
							- 서비스 이용 기록(채용공고 노출·조회·클릭 등 통계 목적의 행위
							기록. 접속 IP 주소를 함께 저장하지 않습니다)
						</LegalParagraph>
						...
						<LegalParagraph>
							회사는 무통장입금 방식만 운영하며, 신용카드 번호·계좌번호 등
							이용자의 결제수단 정보를 수집하거나 저장하지 않습니다. 입금 확인은
							회사 명의 계좌의 입금 내역으로 처리합니다.
						</LegalParagraph>
					</LegalSection>
					...
					<LegalSection heading="2. 개인정보의 처리 목적">
						...
						<LegalParagraph>
							...
							연계정보(CI) 해시값 대조를 통한 중복 가입 및 부정 재가입 방지,
							성별에 따른 서비스 접근 관리(커뮤니티 등), 불량회원의 부정 이용과
							비인가 사용 방지, 고지사항 전달
						</LegalParagraph>

						<LegalSubheading>다. 신고 처리 및 분쟁 대응</LegalSubheading>
						<LegalParagraph>
							이용자의 신고가 접수되거나 법령·이용약관 위반이 의심되는 경우,
							회사는 신고 사실의 확인과 조치를 위해 해당 채팅방의 대화 내용,
							게시물, 문의 내역을 열람할 수 있습니다. 열람은 신고 처리와 분쟁
							해결에 필요한 범위로 한정하며, 그 이력은 별도로 기록·관리됩니다.
						</LegalParagraph>

						<LegalSubheading>라. 서비스 개선 및 통계</LegalSubheading>
						<LegalParagraph>
							접속 빈도 파악, 서비스 이용에 대한 통계 분석, 이를 통한 서비스
							품질 개선
						</LegalParagraph>
						<LegalParagraph>
							회사는 이용자에게 광고성 정보를 전송하거나 맞춤형 광고를 제공하기
							위한 목적으로 개인정보를 처리하지 않습니다. 향후 그러한 처리가
							필요한 경우 별도의 동의를 받겠습니다.
						</LegalParagraph>
					</LegalSection>
					...
					<LegalSection heading="3. 개인정보의 보유 및 이용기간">
						...
						<LegalParagraph>
							탈퇴 신청 즉시 모든 기기의 로그인 세션을 삭제하고 프로필 표시명을
							'탈퇴한 회원'으로, 프로필 사진을 빈 값으로 변경합니다.
						</LegalParagraph>
						<LegalParagraph>
							다만 이용약관에 따라 이용이 제한된 회원의 부정 재가입과 탈퇴
							계정의 재로그인을 막기 위하여, 아이디·이메일 주소·비밀번호 등
							인증정보, 휴대폰번호, 성별, 생년월일과 본인확인
							식별값(연계정보(CI)·중복가입확인 정보(DI)의 일방향 암호화 값)은
							탈퇴일로부터 <PrivacyRetentionDays />일 동안 보관한 뒤 파기합니다.
							이 값은 재가입·재로그인 차단 목적 외에는 이용하지 않습니다. 보관
							기간은 회사의 정책에 따라 변경될 수 있으며, 변경 시 이 처리방침과
							회원 탈퇴 화면에 표시되는 기간이 함께 갱신됩니다.
						</LegalParagraph>
						<LegalParagraph>
							이용자가 작성한 채용공고, 채팅 메시지, 커뮤니티 게시글, 리뷰는
							대화 상대방 등 다른 이용자의 기록을 보호하기 위해 작성자 표시를
							'탈퇴한 회원'으로 바꾼 상태로 유지됩니다.
						</LegalParagraph>

						<LegalSubheading>나. 비회원 성인 인증</LegalSubheading>
						<LegalParagraph>
							비회원의 성인 인증에 사용하는 밤비알바 게스트 쿠키(bambi_guest)는
					...
					<LegalSection heading="4. 개인정보의 제3자 제공">
						<LegalParagraph>
							회사는 이용자의 개인정보를 이 처리방침에서 고지한 범위를 넘어
							외부에 제공하지 않습니다. 다만 이용자가 사전에 동의한 경우, 법령에
							따라 제출 의무가 발생한 경우, 이용자의 생명이나 안전에 급박한
							위험이 확인되어 이를 해소하기 위한 경우에 한하여 개인정보를
							제공합니다.
						</LegalParagraph>
						...
						<LegalParagraph>
							...
							휴대폰번호는 구직 상담을 받기 위한 연락 수단으로 채용공고 상세
							화면에 공개됩니다. 공개를 원하지 않는 경우 채용공고를 등록하지
							않거나 등록된 공고를 삭제할 수 있습니다.
						</LegalParagraph>

						<LegalSubheading>나. 구직자 연락처의 공개</LegalSubheading>
						<LegalParagraph>
							구직자(개인회원)의 휴대폰번호는 공개되지 않습니다. 구인자가
							채팅방에서 연락처 공개를 요청하고 구직자가 이에 동의한 경우에
							한하여 해당 구인자에게만 공개되며, 구직자는 요청을 거절할 수
							있습니다.
						</LegalParagraph>
					</LegalSection>
					...
					<LegalSection heading="5. 개인정보 처리업무의 위탁">
						<LegalParagraph>
							...
							있습니다. 위탁계약 시 「개인정보 보호법」에 따라 위탁업무 수행
							목적 외 개인정보 처리 금지, 안전성 확보조치, 재위탁 제한, 수탁자에
							대한 관리·감독, 손해배상 등 책임에 관한 사항을 문서에 명시하고
							수탁자가 개인정보를 안전하게 처리하는지 감독합니다. 위탁업무의
							내용이나 수탁자가 변경될 경우 지체 없이 이 처리방침을 통해
							공개합니다.
						</LegalParagraph>
						<PrivacyProcessorsTable />
						<LegalParagraph className="pt-2">
							휴대폰 본인확인은 포트원(주)의 본인인증 서비스를 통해 이루어지며,
							실제 본인확인은 인증기관인 엔에이치엔케이씨피(NHN KCP)와 이동통신
					...
					<LegalSection heading="6. 개인정보의 국외 이전">
						<LegalParagraph>
							회사는 서비스 제공에 필요한 범위에서 아래와 같이 개인정보
							처리업무를 국외 사업자에게 위탁하며, 이전되는 정보는 위탁 목적
							범위에서만 처리됩니다.
						</LegalParagraph>
						<LegalTable
							head={[
						...
						/>
						<LegalParagraph className="pt-2">
							이용자는 개인정보의 국외 이전을 거부할 수 있습니다. 거부를 원하는
							경우 회원 탈퇴를 하거나 아래 개인정보 보호책임자에게 요청할 수
							있으며, 웹 이용 통계 목적의 이전은 제7항의 방법으로 쿠키를
							차단하여 거부할 수 있습니다. 다만 서비스 제공에 필수적인 이전을
							거부하는 경우 서비스 이용이 제한될 수 있습니다.
						</LegalParagraph>
					</LegalSection>
					...
					<LegalSection heading="7. 개인정보 자동 수집 장치의 설치·운영 및 그 거부에 관한 사항">
						...
						/>
						<LegalParagraph className="pt-2">
							회사는 구글 애널리틱스(Google Analytics)를 방문 통계 확인
							목적으로만 사용하며, 광고 개인화 및 다른 웹사이트에서의 행태정보
							수집 기능은 사용하지 않도록 설정하고 있습니다.
						</LegalParagraph>
					...
					<LegalSection heading="8. 개인정보의 파기 절차 및 방법">
						<LegalParagraph>
							...
							불필요하게 되었을 때에는 지체 없이 해당 정보를 파기합니다. 다른
							법령에 따라 보존해야 하는 정보는 그 기간 동안 다른 개인정보와
							분리하여 저장·관리한 뒤 파기합니다.
						</LegalParagraph>
						<LegalParagraph>
					...
					<LegalSection heading="11. 개인정보 보호책임자">
						<LegalParagraph>
							...
							피해 구제를 담당하는 개인정보 보호책임자를 지정하고 있습니다.
							개인정보의 열람 청구도 아래 연락처로 접수할 수 있습니다.
						</LegalParagraph>
						<LegalParagraph>
					...
					<LegalSection heading="13. 개인정보 처리방침의 변경">
						<LegalParagraph>
							이 개인정보 처리방침의 내용에 추가, 삭제, 수정이 있을 때에는 시행
							최소 7일 전부터 홈페이지의 '공지사항'을 통해 고지합니다. 다만
							개인정보의 수집 및 이용 목적, 제3자 제공, 국외 이전 등 이용자
							권리에 중요한 변경이 있는 경우에는 최소 30일 전에 고지합니다.
						</LegalParagraph>
					</LegalSection>
				</LegalDoc>
			</BambiScreen>
