...
import {
	...
	type ChatListCopy,
	getChatListCopy,
} from "@/src/lib/chat/chat-audience";
...
function EmptyChats({ copy }: { copy: ChatListCopy }) {
	...
	return (
		<View className="items-center gap-3 py-16">
			...
			<Button
				onPress={() => router.push(copy.emptyCtaHref as Href)}
				size="sm"
				variant="secondary"
			>
				<Button.Label>{copy.emptyCtaLabel}</Button.Label>
			</Button>
		</View>
	);
}
...
function ChatRoomsInner({ audience }: { audience: ChatAudience }) {
	...
	return (
		<BambiScreen>
			<BambiHeader description={copy.description} title="채팅" />
			<ChatRoomsBody
				copy={copy}
...
export function ChatRoomsScreen({ audience }: { audience: ChatAudience }) {
	return (
		<MemberOnly>
			<ChatRoomsInner audience={audience} />
		</MemberOnly>
	);
}
