...
const read = (relative: string) =>
	readFileSync(srcPath(`app/seeker/${relative}`), "utf8");

const layout = read("./layout.tsx");
const page = read("./page.tsx");

describe("seeker 인증 게이트 라우팅", () => {
	it("layout은 공개 수다방 홈을 위해 anon과 guest를 page로 넘긴다", () => {
		expect(layout).toContain('visitor === "anon"');
		expect(layout).toContain('visitor === "guest"');
		expect(layout.match(/return children;/g)).toHaveLength(2);
		expect(layout).not.toContain("<SeekerAuthGateScreen />");
	});

	it("page가 anon과 인증 쿼리 게스트에게 같은 게이트 화면을 그린다", () => {
		// layout은 searchParams를 못 받으므로 이 분기는 반드시 page에 있어야 한다.
		expect(page).toContain("searchParams");
		expect(page).toContain('visitor === "anon"');
