...
//
// 보안 경계: 공고 데이터 중 마스킹되는 건 업소명(company)뿐이고, 그건 서버에서 이미
// 더미 한글로 치환된 BackdropJob으로 들어온다(lib/bambi/auth-backdrop.ts). 지역·급여는
// 공개 랜딩(/jobs)에 이미 노출돼 있어 실값 그대로 실린다. 헤더 로고·내비 라벨 같은 정적
// UI 문구도 공고 데이터가 아니므로 실제 문자열을 쓴다. 광고 배너도 같은 경계다 — 실제 배너 이미지는
// 업소명·문구가 픽셀에 박혀 있어 쓰지 않는다. 이 경계를 넘지 말 것.
//
