...
export const jobLandingTitle = (target: JobLandingTarget): string =>
	target.region
		? `${scopeLabel(target)} 퀸알바·여우알바·밤알바 채용 정보 | 밤비알바`
		: "지역·업종별 퀸알바·여우알바·밤알바 채용 정보 | 밤비알바";
...
export const jobLandingDescription = ({
	...
	if (region && industry) {
		return `${region.label} ${industry.label} 퀸알바·여우알바·밤알바 채용 정보를 모았습니다. 급여와 근무 시간, 업체 인증 여부를 확인하고 밤비알바 1:1 채팅으로 문의하세요.`;
	}

	if (region) {
		return `${region.label} 퀸알바·여우알바·밤알바 관련 유흥·접객 채용 정보를 모았습니다. 룸싸롱·노래주점·단란주점·텐프로/쩜오 등 업종별로 공고를 좁혀 보고 밤비알바 1:1 채팅으로 안전하게 문의하세요.`;
	}

	return "밤비알바의 지역·업종별 퀸알바·여우알바·밤알바 관련 유흥·접객 채용 정보입니다. 룸싸롱·노래주점·단란주점·텐프로/쩜오·BAR 등 전국 시·도와 업종별 공고를 로그인 없이 둘러보세요.";
};
