...
// 댓글 마일스톤 관리 섹션. 등급 관리와 같은 페이지에 두되 자체 쿼리·상태만 쓴다.
function CommentMilestoneSection() {
	...
	const createMutation = useMutation(
		orpc.bambi.memberGrades.commentMilestones.create.mutationOptions({
			onError: (error) =>
				toast.error(localizedGradeError(error.message, MILESTONE_INPUT_ERROR)),
			onSuccess: async () => {
	...
	const removeMutation = useMutation(
		orpc.bambi.memberGrades.commentMilestones.remove.mutationOptions({
			onError: (error) =>
				toast.error(
					localizedGradeError(error.message, "마일스톤을 삭제하지 못했어요.")
				),
			onSuccess: async () => {
