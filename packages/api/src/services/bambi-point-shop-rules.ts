// 포인트몰의 순수 판정·상수. native(apps/native)가 이 파일을 직접 import하므로
// drizzle·db·schema import를 여기 두지 않는다 — 두면 RN 번들에 ORM이 딸려 들어간다.
// db가 필요한 것(락·재고 차감·원자 갱신)은 bambi-point-shop.ts에 남기고, 그 파일이
// 여기 심볼을 그대로 재수출해 서버·web의 기존 import 경로를 유지한다.
