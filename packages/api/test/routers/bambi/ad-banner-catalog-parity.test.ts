...
// 패턴(apps/web/test/components/bambi/visual-job-components.test.ts)대로 소스를 읽어 값을 뽑는다.
//
// 대조 대상이 packages/api/src 밖에 있어 srcPath에서 리포 루트로 거슬러 올라간다 —
// 앵커를 테스트 파일 깊이가 아니라 src에 두면 테스트를 옮겨도 경로가 따라오지 않는다.
const CATALOG_SOURCE = fs.readFileSync(
	srcPath("../../../apps/web/src/lib/bambi/ad-banner-layout.ts"),
	"utf8"
);
